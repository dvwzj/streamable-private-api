...
class StreamablePrivateAPI {
        ...
        try {
          _this.http.post('https://ajax.streamable.com/check', {
            ...
            _this.http.defaults.headers = _lodash.default.merge(_this.http.defaults.headers, {
              'cookie': "user_name=".concat(res.cookies.user_name, "; user_code=").concat(res.cookies.user_code, "; session=").concat(res.cookies.session)
            });
            resolve(_this);
          }).catch(reject);
        } catch (e) {
  ...
  forceUpload(url, title, source, extract_id) {
    ...
    try {
      var plan_max_length = this.auth.plan_max_length || 600; // const query = querystring.stringify({
      //   url
      // })

      title = title === undefined ? '' : title;
      this.http.get("https://ajax.streamable.com/extract", {
        params: {
